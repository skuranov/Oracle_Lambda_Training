...
public class Lesson3 {
    ...
    static int[][] computeLevenshtein(List<String> wordList, boolean parallel) {
        ...
        IntStream stream = parallel ? IntStream.range(0, wordList.size() - 1).parallel() : IntStream.range(0, wordList.size() - 1);

        stream.forEach(
                x ->
                {
                    for (int i = 0; i < wordList.size(); i++) {
                        distances[x][i] = Levenshtein.lev(wordList.get(x), wordList.get(i));
                    }
                });
